...
void displayimage032(uint8 *p) 
{
    ...
    {
        Lcd_SetRegion(i,0,i,image_col-1);		//坐标设置
        for(j=0;j<image_col;j++)
        {	
			temp = *(p + i*image_col + (image_col-1)-j);//读取像素点
...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      显示二值化图片加中线  边界为绿色   中线为黑线
//  @param      *p     			         图像数组地址
//  @return     void
//  @since      v2.0
//  @note      	图像分辨率需要设置在128(高)*160(宽)以下      
//              图片数组             ROW 0-119  COL 0-187     
//              由于屏幕分辨率只有128*160，故COL取 14-173  
//-------------------------------------------------------------------------------------------------------------------
void display_img_bin()
{
    int x,y;
    for(x = 0; x < image_row; ++x)
    {
      Lcd_SetRegion(x, 0, x, image_col-1);
      for(y = image_col-1; y >= 0 ; --y)
      {
        if(y == edge_right[x])LCD_WriteData_16Bit(GREEN);
        else if(y == edge_left[x] && x > DivLine)LCD_WriteData_16Bit(BRRED);
        else if(image_midline[x] == y && x > DivLine)LCD_WriteData_16Bit(0x0000);             
        else if(image_bin[x][y] == 1)LCD_WriteData_16Bit(0xffff);
        else LCD_WriteData_16Bit(0x0000);
      }
    }
}
