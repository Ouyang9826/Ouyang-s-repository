...
//  @return     void
//  @since      v1.0
//  Sample usage:               ftm_pwm_init(ftm0,ftm_ch0,50,5000);		//ftm0模块，0通道，50HZ，百分之50的占空比
//-------------------------------------------------------------------------------------------------------------------
...
//输入左编码器输入捕获
/*-------------------------------------------------------------------------------------------------------------*/
void FTM1_input_encoder(void)
{
  FTM_port_mux(ftm1, ftm_ch0);//初始化端口
  ...
  port_init_NoAlt(FTM1_CH0_PIN,PULLUP);//PTA12引脚用于捕获编码器的脉冲，因为要捕获下降沿，所以此处配上拉电阻
                                       //中断服务函数
  enable_irq(FTM1_IRQn);//开启FTM1的输入捕获中断  FTM1_IRQHandler
  // EnableInterrupts;//开总中断            在这儿不开也行
  //gpio_init(FTM1_CH1_PIN,GPI,0);   //初始化A5为gpio模式，用于接收左侧编码器的旋转方向标志位，编码器顺时针转输出高电平，逆时针转输出低电平
}

//输入右编码器输入捕获
/*-------------------------------------------------------------------------------------------------------------*/
void FTM2_input_encoder(void)
{
  FTM_port_mux(ftm2, ftm_ch0);//初始化端口
  ftm_input_init(ftm2,ftm_ch0,FTM_Falling,5000);//输入捕获初始化    右电机测速  下降沿触发    5000HZ
  port_init_NoAlt(FTM2_CH0_PIN,PULLUP);//PTB18引脚用于捕获编码器的脉冲，因为要捕获下降沿，所以此处配上拉电阻                                PTB18
                                       //中断服务函数
  enable_irq(FTM2_IRQn);//开启FTM1的输入捕获中断  FTM1_IRQHandler
  // EnableInterrupts;//开总中断            最好不要开
}
